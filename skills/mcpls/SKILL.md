...
5. When inspecting several known handles, call `inspect_symbol` with `targets` once instead of repeating it. It preserves every target identity while sharing one response and provider budget.
