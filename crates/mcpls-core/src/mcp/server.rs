...
"#;
    ...
    #[tokio::test]
    async fn resources_read_deferred_code_action_details_reassemble_losslessly() {
        let server = create_test_server_with_project().await;
        let actions = (0..130)
            .map(|index| {
                serde_json::json!({
                    "action_id": format!("action-{index}"),
                    "title": format!("Fix {index}"),
                    "command": {"command": "fix", "arguments": ["λ".repeat(2_000)]}
                })
            })
            .collect::<Vec<_>>();
        let expected = serde_json::to_string(&actions).unwrap();
        let reference = server
            .context
            .project_registry
            .store_deferred_resource(
        ...
        loop {
            ...
            let SessionResource::Deferred(deferred) = resource else {
                panic!("expected deferred code-action resource");
            };
            let response = server
                .read_deferred_resource(deferred, uri.clone(), false)
                .unwrap();
            let ReadResourceResponse::Complete(response) = response else {
                panic!("deferred code-action resource unexpectedly requested input");
            };
            let ResourceContents::TextResourceContents { text, .. } = &response.contents[0] else {
                panic!("deferred code-action resource was not text");
            };
            let page: SemanticResourceReadResult = serde_json::from_str(text).unwrap();
            assert!(serde_json::to_vec(&page).unwrap().len() <= MAX_SEMANTIC_RESOURCE_RESULT_BYTES);
            recovered.push_str(&page.text);
    ...
    async fn resources_read_deferred_signature_documentation_reassembles_losslessly() {
        ...
        let signatures = (0..70)
            .map(|index| {
                serde_json::json!({
                    "signature_id": format!("signature-{index}"),
                    "label": format!("fn item_{index}()"),
                    "documentation": "λ".repeat(700),
                    "parameters": [{"label": [0, 4], "documentation": "parameter"}]
                })
            })
            .collect::<Vec<_>>();
        let expected = serde_json::to_string(&signatures).unwrap();
        let reference = server
            .context
            .project_registry
            .store_deferred_resource(
        ...
        loop {
            let resource = parse_session_resource_uri(&uri).unwrap();
            let SessionResource::Deferred(deferred) = resource else {
                panic!("expected deferred signature resource");
            };
            let response = server
                .read_deferred_resource(deferred, uri.clone(), false)
                .unwrap();
            let ReadResourceResponse::Complete(response) = response else {
                panic!("deferred signature resource unexpectedly requested input");
            };
            let ResourceContents::TextResourceContents { text, .. } = &response.contents[0] else {
                panic!("deferred signature resource was not text");
            };
            let page: SemanticResourceReadResult = serde_json::from_str(text).unwrap();
            assert!(serde_json::to_vec(&page).unwrap().len() <= MAX_SEMANTIC_RESOURCE_RESULT_BYTES);
            recovered.push_str(&page.text);
...
"#;
    ...
    #[tokio::test]
    async fn server_log_pages_and_deferred_resources_replay_losslessly() {
        let server = create_test_server_with_project().await;
        let project_id = ProjectId::new("project").unwrap();
        let actor = server
            .context
            ...
            actor
                .notify(
                    ...
                    crate::lsp::LspNotification::parse(
                        "window/logMessage",
                        Some(serde_json::json!({"type": 3, "message": format!("log-{index}")})),
                    ),
                )
                .await
                .unwrap();
        ...
        actor
            .notify(
                ...
                crate::lsp::LspNotification::parse(
                    ...
                    Some(serde_json::json!({
                        "type": 1,
                        "message": "oversized log ".repeat(2_000)
                    })),
                ),
            )
            .await
        ...
        loop {
            ...
            for log in logs {
                assert!(seen.insert(log.to_string()), "duplicate log: {log}");
                if let Some(uri) = log["message_resource"]["uri"].as_str() {
        ...
        assert!(pages > 1);
        let uri = deferred_uri.expect("oversized log should have a resource");
        let semantic = server
        ...
        assert!(semantic.text.contains("oversized log"));

        let SessionResource::Deferred(deferred) = parse_session_resource_uri(&uri).unwrap() else {
            panic!("expected deferred log resource");
        };
        let response = server.read_deferred_resource(deferred, uri, false).unwrap();
        let ReadResourceResponse::Complete(response) = response else {
            panic!("deferred log resource unexpectedly requested input");
        };
        let ResourceContents::TextResourceContents { text, .. } = &response.contents[0] else {
            panic!("deferred log resource was not text");
        };
        let page: SemanticResourceReadResult = serde_json::from_str(text).unwrap();
    ...
    #[tokio::test]
    async fn notification_cursor_rejects_a_concurrent_append() {
        let server = create_test_server_with_project().await;
        let project_id = ProjectId::new("project").unwrap();
        let actor = server
            .context
