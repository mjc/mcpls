...
pub use config::{DaemonConfig, ProjectConfigTrust, ServerConfig};
pub use error::Error;
...
pub async fn serve_with(config: ServerConfig, transport: Transport) -> Result<(), Error> {
    ...
    let project_registry =
        project::ProjectRegistry::with_translator_template(32, translator_template)
            ...
            .with_rust_residency_limit(config.daemon.rust_analyzer_resident_groups);
        ...
        async fn test_serve_degrades_when_all_servers_fail_to_spawn() {
            ...
            let config = ServerConfig {
                workspace: WorkspaceConfig {
                    ...
                    max_documents: crate::bridge::DEFAULT_MAX_DOCUMENTS,
                    max_file_size: crate::bridge::DEFAULT_MAX_FILE_SIZE,
                },
